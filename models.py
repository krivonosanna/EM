...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        """
        answ = []
        for obj in parallel_corpus:
            src = obj.source_tokens
            tar = obj.target_tokens
            buf = self.translation_probs[src[:, np.newaxis], tar[np.newaxis, :]]
            answ.append(buf / buf.sum(axis=0))
        return answ
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        """
        s = 0
        for k in range(len(parallel_corpus)):
            obj = parallel_corpus[k]
            src = obj.source_tokens
            tar = obj.target_tokens
            m = len(tar)
            n = len(src)
            ...
            buf2 = posteriors[k]
            buf2[buf2 == 0] = 1
            s += np.sum(posteriors[k] * (np.log(buf) - np.log(buf2))) - m * np.log(n)
        return s
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for k in range(len(parallel_corpus)):
            ...
            tar = obj.target_tokens
            s = posteriors[k]
            np.add.at(self.translation_probs, (src[:, np.newaxis], tar[np.newaxis, :]), s)
    ...
    def align(self, sentences):
        q = self._e_step(sentences)
        result = []
        ind = np.arange(1, self.num_target_words + 1)
        for k in range(len(sentences)):
            result.append(list(zip(np.argmax(q[k], axis=0) + 1, ind[:q[k].shape[1]])))
        return result
